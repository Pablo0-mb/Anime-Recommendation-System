...
def get_dataframe():
    ...
    # CAMBIO: Usar asignación en lugar de inplace=True para futuras versiones de pandas
    puntajes['rating'] = puntajes['rating'].replace(to_replace = -1, value = 0) # Asume 'rating' es la columna original
    
    # Mezcla los dos DataFrames
    animes_puntaje = puntajes.merge(animes, left_on = 'anime_id',right_on = 'anime_id',suffixes=['_user',''])
    
    # Mezcla (reordena aleatoriamente)
...
def plot_rating_distribution():
    ...
    with sns.axes_style('white'):
        # CAMBIO: Especificar explícitamente 'x=' para evitar TypeError
        g = sns.catplot(x="rating_user", data=animes_puntaje, aspect=2.0, kind='count')
        g.set_ylabels("Total number of ratings")
        g.savefig('puntajes.png')

def save_dicts(id_to_anime_name, anime_name_to_id):
    # MEJORA: Añadir extensión .json para claridad
    with open('id_to_anime_name.json','w') as f:
        json.dump(id_to_anime_name,f)
        
    with open('anime_name_to_id.json','w') as f :
        json.dump(anime_name_to_id,f)
        

def load_dict(name):
    try:
        # MEJORA: Esperar que el archivo tenga extensión .json
...
def load_anime_dicts():
    # Las llamadas a load_dict ya se encargan del .json
